...
/** Recusas comuns, para cada frase existir num lugar só. */
const NAO_E_SEU = "Você não tem permissão para publicar ou despublicar este cão.";
const NAO_E_SEU_CANIL = "Você não tem permissão para publicar ou despublicar este canil.";

/**
 * Espelha `private.can_manage_dog` MENOS o ramo de admin — e é essa subtração
 * que é o ponto.
 *
 * `publishDog`/`unpublishDog` nunca filtraram posse: atualizavam por id e
 * deixavam a decisão para a RLS. Só que `dogs_update` carrega
 * `or private.is_admin()`, então o caminho do DONO aceitava um admin
 * publicando QUALQUER cão do banco — sem auditoria nenhuma, porque
 * `private.audit()` não é alcançável daqui. Não era hipótese remota: bastava um
 * admin abrir `/painel/caes/<id>` de outra pessoa.
 *
 * O admin agora tem porta própria e auditada (`setDogPublishedByAdmin`, em
 * `modules/admin/actions.ts`). Esta função devolve a este arquivo o que o nome
 * dele sempre prometeu: o caminho do dono.
 *
 * Os três ramos vêm de `can_manage_dog` e são todos necessários — `created_by`
 * cobre quem cadastrou, e `kennels.owner_id` cobre o dono do canil quando o cão
 * tem `owner_id` de outra pessoa (o caso que o cadastro por admin cria).
 */
function ehDonoDoCao(
  dog: { owner_id: string | null; created_by: string | null; kennels?: unknown },
  userId: string,
): boolean {
  const k = dog.kennels as { owner_id: string } | { owner_id: string }[] | null | undefined;
  const donoDoCanil = Array.isArray(k) ? (k[0]?.owner_id ?? null) : (k?.owner_id ?? null);
  return dog.owner_id === userId || dog.created_by === userId || donoDoCanil === userId;
}
...
export async function publishKennel(formData: FormData): Promise<PublishState> {
  ...
  const user = await requireUser(`/painel/canis/${id}`);
  ...
  // Mesmo buraco de `publishDog`, mesma correção: `kennels_update_own` aceita
  // `or private.is_admin()`, então sem esta linha o caminho do dono publicava
  // canil alheio para qualquer admin, sem rastro. Canil não tem `created_by`
  // relevante aqui — a posse é `owner_id`, e ponto.
  if (kennel.owner_id !== user.id) return { error: NAO_E_SEU_CANIL };
...
export async function publishDog(formData: FormData): Promise<PublishState> {
  ...
  const { data: dog } = await supabase
    .from("dogs")
    .select("id, public_id, kennel_id, owner_id, created_by, kennels(slug, owner_id)")
    .eq("id", id)
    .is("deleted_at", null)
    .maybeSingle();
  if (!dog) return { error: "Cão não encontrado." };
  if (!ehDonoDoCao(dog, user.id)) return { error: NAO_E_SEU };
