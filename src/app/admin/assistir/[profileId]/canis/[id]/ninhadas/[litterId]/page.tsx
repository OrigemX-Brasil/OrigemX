import Painel from "@/app/(app)/painel/canis/[id]/ninhadas/[litterId]/page";
 ...
 */
export default async function AssistirPage({ params }: {
  params: Promise<{ profileId: string; id: string; litterId: string }>;
}) {
  const { id, litterId } = await params;
  return <Painel params={Promise.resolve({ id, litterId })} />;
}
